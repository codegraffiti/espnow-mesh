...
#define KEY_GRACE_PERIOD_MS 3600000        // 1 hour grace period
...
void ESPNowMesh::loop() {
  ...
  if (role_ == ROLE_COORDINATOR && !network_key_previous_.empty() && 
      millis() - key_rotation_grace_start_ >= KEY_GRACE_PERIOD_MS) {
    network_key_previous_.clear();
    ESP_LOGI(TAG, "Previous key cleared after grace period");
  }
  ...
  if (role_ == ROLE_END_DEVICE && node_joined_) {
    ...
    if (millis() - last_wake >= AWAKE_DURATION_MS + SLEEP_DURATION_MS) {
      std::vector<float> values = {25.5, 60.2, 500.0, 1.0, 0.0};
      send_data(1, values);
      last_wake = millis();
  ...
  do {
    ...
    if (retries == MAX_RETRIES) {
      ...
      if (local_buffer_.size() < MAX_BUFFER_SIZE) {
        ...
        ESP_LOGW(TAG, "Message queued");
      } else {
        ESP_LOGE(TAG, "Message dropped");
      }
...
void ESPNowMesh::process_join_request_(const MeshMessage &msg, const uint8_t *mac_addr) {
  ...
  if (banned_nodes_.find(new_node_id) != banned_nodes_.end()) return;
  ...
  node_joined_ = true;
  ESP_LOGI(TAG, "Node %02X:%02X:%02X joined (pending auth verification)", new_node_id[0], new_node_id[1], new_node_id[2]);
}
